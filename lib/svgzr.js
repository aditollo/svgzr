...
'use strict';
	...
	var firstCycle = function(options) {
		...
		result = result.then(function() {
			if(options.png) {
				mkdirp(options.files.cwdPng, function(err) {
		...
		return result.then(function() {
			...
			if(options.svg && filesSvg.length !== 0) {
				...
				mkdirp(path.dirname(options.svg.destFile), function(err) {
	...
	var createFallback = function(options) {
		...
		if(filesFallback.length !== 0) {
			...
			mkdirp(path.dirname(options.fallback.destFile), function(err) {
	...
	module.exports = function svgzr(newOptions, callback) {
		...
		if(!options.templateFileSvg) {
			options.templateFileSvg = path.join(__dirname, '..', 'templates', 'templateSvg.mst');
		}
		if(!options.templateFileFallback) {
			options.templateFileFallback = path.join(__dirname, '..', 'templates', 'templateFallback.mst');
		}
